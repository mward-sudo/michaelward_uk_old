defmodule MichaelwardUkWeb.Hooks.User do
  @moduledoc """
  Session hook for setting a user assign
  """
  require Logger
  use MichaelwardUkWeb, :live_view
  alias MichaelwardUk.Accounts.User
  alias MichaelwardUk.Authentication

  @doc """
  Assign a `current_user` key to the socket.

  If the socket is connected, a user lookup is attemptedd, using `_user_token` from `app.js`
  """
  ...
  def on_mount(:default, _, _, socket) do
    with true <- connected?(socket),
      ...
      {:cont, socket |> assign(:current_user, current_user)}
    else
      false ->
        # Socket us not yet connected
